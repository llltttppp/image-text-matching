...
class BidirectionNet:
    ...
    def train_multidataset(self, sess, maxEpoch=300, lr=0.0001,is_load=False,ckpt_path='',only_image = False):
        logdir = './log/Bidirectionnet_GMM/GMM_ourword2vec/sigmod9000/'
        print 'log in %s' %logdir
        model_save_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/sigmod9000/'
        make_if_not_exist(model_save_path)
        model_save_path += 'model'
        data_root = '/media/ltp/40BC89ECBC89DD32/souhu_fusai/'
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        if is_train:
            ...
            model.train_multidataset(sess,lr=0.001)#,is_load=True,ckpt_path='./model/Bidirectionnet_GMM/GMM_30norm/model-5000')
            ...
            model_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/sigmod9000/model-2500'


            scope ='sentence'
            save_path ='./emb/train_sentence_embed_GMM_ourword2vec_sigmod9000_2500.h5'
            model.test_embed(sess, feat_file, model_path, scope, save_path)	
            feat_file = './test_img_feat_3crop_mean.h5'
            scope='image'
            save_path ='./emb/train_image_embed_GMM_ourword2vec_sigmod9000_2500.h5'
            model.test_embed(sess, feat_file, model_path, scope, save_path)	
