...
class BidirectionNet:
    ...
    def lstm(self, input_tensor, reuse=False):
       ...
       # with tf.variable_scope('lstm', reuse=reuse) as scope:
        #    lstm_cell = tf.contrib.rnn.BasicLSTMCell(300)
         #   zero_state = lstm_cell.zero_state(
          #      batch_size=input_tensor.get_shape()[0], dtype=tf.float32)
           ...
           # lstm1,_ = tf.contrib.rnn.static_rnn(lstm_cell, inputs=input_list, initial_state=zero_state)
            #lstm_output = tf.expand_dims(tf.stack(lstm1,axis=1),axis=1)
       # self.endpoint['LSTM_output'] = lstm_output
        with tf.variable_scope('lstm_conv', reuse=reuse) as scope:
	    conv1 = self.conv_layer(tf.expand_dims(input_tensor,axis=1),512,[1,3],1)
	    conv1 = self.conv_layer(conv1,512,[1,3],1)
	    conv1 = tf.nn.max_pool(conv1,[1,1,2,1],[1,1,2,1],padding='SAME')
	    conv2 =self.conv_layer(conv1,256,[1,2],1)
	    conv2 = self.conv_layer(conv2,256,[1,2],1)
	    conv2 = tf.nn.max_pool(conv2,[1,1,2,1],[1,1,2,1],padding='SAME')
            lstm_result=tf.reshape(conv2,[self.batch_size,-1])    
    ...
    def build_unite(self, loss_weight=0):
	lstm_embed =self.lstm(self.raw_sentence)
    ...
    def train(self, sess, maxEpoch=300, lr=0.0001,is_load=False,only_sentence=False,is_fixsentence=False, ckpt_path=''):
        logdir = './log/Bidirectionnet_lstm/conv_25*256/'
        model_save_path='./model/Bidirectionnet_lstm/conv25*256/'
	make_if_not_exist(model_save_path)
	model_save_path += 'model'
	root = '../souhu_data/fusai/'
	t_var = tf.trainable_variables()
	if is_load:
	    var_list = [var for var in t_var if 'lstm' in var.name]
	...
	train_op = self.build_trainop(self.total_loss, lr=lr, t_vars = t_vars)
	...
	print 'repeat %d. %d iters/epoch. %d iters/(epoch*repeat)' %(REPEAT, iters_per_epoch, 
		...
		for _ in range(REPEAT):
			...
			# train
			feed_dict = {self.raw_sentence: raw_sentence, self.image_feat: img_feat}
			...
			if np.mod(step, 1000) == 0:
			    ...
			    print 'model saved to %s-%d' %(model_save_path, step)
			if np.mod(step, 20) == 0:
			    t = (time.time() - t0)/3600
	...
	labels = joblib.load(root + 'train/kmeans_docVec_1000.model').labels_
        train_op =self.build_trainop(self.total_loss, lr=lr, clipping_norm=50, t_vars=tf.trainable_variables())
        summary_writer = tf.summary.FileWriter(logdir, sess.graph)
        summary_op = tf.summary.merge_all()
        N = labels.shape[0]
    ...
    def test_embed(self, sess, feat_file, model_path, scope, save_path, h5dataset='embed', batch_size=100):
	    ...
	    if scope=='sentence':
		feat_batch=self.read_wordvector(feat_all[interval],self.batch_size)
	    else:
...
if __name__ == '__main__':
    ...
    ckpt_path = '/media/wwt/860G/model/tf_souhu/lstm_crossEnt_T1/ckpt-3000'
    config=tf.ConfigProto()
    config.gpu_options.allow_growth=True
    with tf.Session(config=config) as sess:
        model = BidirectionNet(is_training=True, batch_size = 2000 ) # 
        if is_train:
            model.train(sess, is_load=is_load, ckpt_path=ckpt_path)
            #model.train_softmax(sess, is_load=is_load, ckpt_path=ckpt_path)
        else:
            feat_file = './test_txt/test_cont.txt'
	    #feat_file = '/media/ltp/40BC89ECBC89DD32/souhu_fusai/test_img_feat_3crop_mean.h5'
            model_path = './model/Bidirectionnet_conv/conv1/model-30000'
            scope = 'sentence'
            #scope='image'
            #save_path ='./emb/train_image_embed_conv_30000.h5'
            save_path = './emb/train_sentence_embed_conv_30000.h5'
