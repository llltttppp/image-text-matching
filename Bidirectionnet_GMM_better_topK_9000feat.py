...
class BidirectionNet:
    ...
    def train_multidataset(self, sess, maxEpoch=300, lr=0.0001,is_load=False,ckpt_path='',only_image = False):
        logdir = './log/Bidirectionnet_GMM/GMM_ourword2vec/wwttopK9000feat_100words/'
        print 'log in %s' %logdir
        model_save_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/wwttopK9000feat_100words/'
        make_if_not_exist(model_save_path)
        model_save_path += 'model'
        data_root = '/media/ltp/40BC89ECBC89DD32/souhu_fusai/'
        img_feat_file = data_root + 'train_img_feat_3crop_mean_all.h5'
        sentence_feat_file =data_root+'train_sentence_fishervectors_ourword2vec_100words.h5'
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        if is_train:
            ...
            model_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/wwttopK9000feat/model-8500'
            scope ='sentence'
           
            save_path = './emb/train_sentence_embed_GMM_ourword2vec_wwttopK9000feat_8500.h5'
            model.test_embed(sess, feat_file, model_path, scope, save_path)	
            feat_file = './test_img_feat_3crop_mean.h5'
            scope='image'
            save_path ='./emb/train_image_embed_GMM_ourword2vec_wwttopK9000feat_8500.h5'
            model.test_embed(sess, feat_file, model_path, scope, save_path)	
