...
class BidirectionNet:
    ...
    def build_input(self):
        # positive
        self.cluster_feature = tf.placeholder(tf.float32, shape=[self.batch_size,512],name='cluster_feature')
    ...
    def build_matchnet(self):
        self.sentence_fc2 = self.sentencenet(self.sentence_emb, reuse=False,skip=self.is_skip)
        self.image_fc2 = self.imagenet(self.image_feat, reuse=False)
        # compute loss
        if self.is_training:
            self.reg_loss = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
            if not self.is_TopKloss:
                self.total_loss=tf.add_n(list(self.faketop_K_loss(self.sentence_fc2,self.image_fc2))+self.reg_loss)
            else:     
                if self.is_skip:
                    self.total_loss =tf.add_n([self.positive_loss(self.sentence_fc2,self.image_fc2)]+list(self.top_K_loss(self.sentence_fc2,self.image_fc2))+self.reg_loss)
                else:
    ...
    def faketop_K_loss(self,sentence,image,K=50,margin=0.3):
        sim_matrix = tf.matmul(sentence, image, transpose_b=True)
        s_square = tf.reduce_sum(tf.square(sentence), axis=1)
        im_square = tf.reduce_sum(tf.square(image), axis=1)
        d = tf.reshape(s_square,[-1,1]) - 2 * sim_matrix + tf.reshape(im_square, [1, -1])
        ...
        length = tf.shape(d)[-1]
        d = tf.matrix_set_diag(d, 8 * tf.ones([length]))
        sen_loss_K = tf.nn.top_k(-1.0 * d, 2*K, sorted=True)[0][:,-K:] # note: this is negative value
        im_loss_K = tf.nn.top_k(tf.transpose(-1.0 * d), 2*K, sorted=True)[0][:,-K:] # note: this is negative value
        sentence_center_loss = tf.nn.relu(positive + sen_loss_K + margin)
        image_center_loss = tf.nn.relu(positive + im_loss_K + margin)
        self.d_neg = tf.reduce_mean((sen_loss_K + im_loss_K)/-2.0)
        self.d_pos =tf.reduce_mean(positive)
    ...
    def train_multidataset(self, sess, maxEpoch=300, lr=0.0001,is_load=False,ckpt_path='',only_image = False):
        logdir = './log/Bidirectionnet_GMM/GMM_ourword2vec/cluster_skip/'
        print 'log in %s' %logdir
        model_save_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/cluster_skip/'
        make_if_not_exist(model_save_path)
        model_save_path += 'model'
        data_root = '/media/ltp/40BC89ECBC89DD32/souhu_fusai/'
        ...
        img_feat_all=[]
        sentence =[]
        cluster_feat=cluster_h5['feature'][:]
        t0 = time.time()
        for epoch in range(maxEpoch):
            for setId in np.random.permutation(nDataset):
                ...
                if setId < nDataset - 1:
                    ...
                    if not self.is_skip:
                        sentence = sen_h5['feature'][shift:shift+dataset_size,:9000]
                    cluster = cluster_feat[shift:shift+dataset_size,:]
                    N = img_feat_all.shape[0]                     
                    assert N == dataset_size
                else: 
                    img_feat_all = img_h5['feature'][shift:,:]
                    if not self.is_skip:
                        sentence = sen_h5['feature'][shift:,:9000]
                    cluster = cluster_feat[shift:,:]
                    N = img_feat_all.shape[0] 
                print 'load dataset No%d  %d samples'%(setId,N)
                for _ in range(1):
                    idxArr = np.random.permutation(N)
                    batch_idx = int(N / self.batch_size)
                    for idx in range(batch_idx):
                        ...
                        c_feat = cluster[idxArr[interval]]
                        ...
                        if not self.is_skip:
                            feed_dict = {self.raw_sentence:raw_sentence, self.image_feat: image_feat,self.cluster_feature:c_feat}
                        else:
                            feed_dict = {self.image_feat: image_feat,self.cluster_feature:c_feat}
    ...
    def test_embed(self, sess, feat_file, model_path, scope, save_path, h5dataset='embed',cluster_file='./test_tfidf_word2vec_cluster.h5'):
        '''
        ...
        if scope == 'image':
            ...
            h5file = h5py.File(feat_file, 'r')
            feat_all = h5file['feature'][:,:9000]
            cluster_feat = h5py.File(cluster_file, 'r')['feature'][:]
        else:
        ...
        if np.mod(N, self.batch_size) != 0:
            x=self.batch_size-np.mod(N, self.batch_size)
            feat_all =np.concatenate([feat_all,np.zeros((x,)+feat_all.shape[1:])],axis=0)
            N=N+x
        # load model
        ...
        for idx in range(N/self.batch_size):
            ...
            if scope == 'sentence':
                ...
                if not self.is_skip:
                    feed_dict = {input_tensor: feat_batch,self.cluster_feature:cluster_feat[interval]}
                else:
                    feed_dict = {self.cluster_feature:cluster_feat[interval]}
            else:
                feat_batch = feat_all[interval]
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        if is_train:
            model = BidirectionNet(is_training=True,is_skip=True,is_TopKloss =True,batch_size=2000,is_keep_prob=False)
            ...
            feat_file =['./test_img_feat_3crop_mean.h5','./test_sentence_fishervectors_ourword2vec.h5']*2
         
            model_path = ['./final_model/9000cluster/model-20000', './final_model/9000cluster/model-20000','./final_model/faketopK_9000cluster/model-10000','./final_model/faketopK_9000cluster/model-10000']
            scope =['image','sentence']*2

           
            #save_path ='./emb/train_image_embed_GMM_ourword2vec_9000cluster_20000.h5'
            save_path = ['./emb/train_image_embed_GMM_ourword2vec_9000cluster_20000.h5','./emb/train_sentence_embed_GMM_ourword2vec_9000cluster_20000.h5',
                         './emb/train_image_embed_GMM_ourword2vec_faketopK_9000cluster_10000.h5','./emb/train_sentence_embed_GMM_ourword2vec_faketopK_9000cluster_10000.h5'
                         ]
