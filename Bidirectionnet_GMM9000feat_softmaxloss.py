...
class BidirectionNet:
    ...
    def build_matchnet(self):
        ...
        self.image_fc2,self.logits = self.imagenet(self.image_feat, reuse=False,skip=self.is_skip)
        self.softmaxloss =0.1*tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(
                                                                               labels=self.labels, 
                                                                               logits=self.logits))
        # compute loss
        if self.is_training:
            ...
            if not self.is_TopKloss:
                ...
                self.total_loss =tf.add_n(list(self.top_K_loss(self.sentence_fc2,self.image_fc2))+self.reg_loss+[self.softmaxloss])
    ...
    def train_multidataset(self, sess, maxEpoch=300, lr=0.0001,is_load=False,ckpt_path='',only_image = False):
        logdir = './log/Bidirectionnet_GMM/GMM_ourword2vec/9000feat_softmax/'
        print 'log in %s' %logdir
        model_save_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/9000feat_softmax/'
        make_if_not_exist(model_save_path)
        model_save_path += 'model'
        data_root = '/media/ltp/40BC89ECBC89DD32/souhu_fusai/'
        ...
        news_all = np.array([v.strip() for v in open('./train_txt/train.txt').readlines()])
        ...
        for epoch in range(maxEpoch):
            for setId in np.random.permutation(nDataset):
                ...
                if setId < nDataset - 1:
                    ...
                    news = news_all[shift:shift+dataset_size]
                ...
                for _ in range(1):
                    ...
                    for idx in range(batch_idx):
                        ...
                        labels=self.doc2vector(news[idxArr[interval]])
                        ...
                        feed_dict = {self.raw_sentence:raw_sentence, self.image_feat: image_feat,self.labels:labels}
    ...
    def test_embed(self, sess, feat_file, model_path, scope, save_path, h5dataset='embed'):
        ...
        load_var = [var for var in t_var if 'softmax' not in var.name]
    ...
    def doc2vector(self,doc_list,max_words=1):
        #ss=time.time()
        labels =np.zeros((self.batch_size,self.num_class),dtype=np.float32)
        count=0
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        if is_train:
            ...
            model.train_multidataset(sess,lr=0.001)#is_load=True,ckpt_path='./model/Bidirectionnet_GMM/GMM_ourword2vec/wwttopK9000feat_100words/model-5500')
            ...
            #model_path = './final_model/wwttopK9000feat/model-8500'
            model_path = './model/Bidirectionnet_GMM/GMM_ourword2vec/9000feat_softmax/model-8500'
            scope ='sentence'
           
            save_path = './emb/train_sentence_embed_GMM_ourword2vec_9000feat_softmax_8500.h5'
            model.test_embed(sess, feat_file, model_path, scope, save_path)	
            feat_file = './test_img_feat_3crop_mean.h5'
            scope='image'
            save_path ='./emb/train_image_embed_GMM_ourword2vec_9000feat_softmax_8500.h5'
            model.test_embed(sess, feat_file, model_path, scope, save_path)	
